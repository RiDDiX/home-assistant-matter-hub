...
| **Alpha** | `alpha` | v2.1.0-alpha.887 | Ahead of Stable, carries the additions listed under Alpha Features |
...
- 🧹 **A filtered-out vacuum keeps its room switches' numbers**: with room switches on, editing the filter to drop the vacuum deleted the switches outright, so putting it back re-added them under new numbers and controllers lost their groups. The switches now leave with the vacuum and return on their old numbers ([#468](https://github.com/RiDDiX/home-assistant-matter-hub/issues/468))
