...
| **Alpha** | `alpha` | v2.1.0-alpha.889 | Ahead of Stable, carries the additions listed under Alpha Features |
...
- 🧠 **Heap limit is configurable**: the add-on takes a `heap_size_mb` option, the automatic sizing moved from a quarter to half of the available memory, and a `NODE_OPTIONS` you set yourself is honored instead of being silently overridden ([#459](https://github.com/RiDDiX/home-assistant-matter-hub/issues/459))
- 🧩 **Third-party plugins load again**: an installed plugin package is imported through its manifest entry point instead of its directory, plugin devices report their own name to controllers, and adding or removing one tells commissioned controllers that the bridge composition changed; contributed by Patrick Gu ([#458](https://github.com/RiDDiX/home-assistant-matter-hub/pull/458))
...
- 🏷️ **A device renamed in Home Assistant reaches the controller**: the registry snapshot is pushed to the endpoints on every registry refresh, a rename counts as a registry change now so that refresh runs at all, and server mode pushes the renamed identity to its root node. Apple Home takes the accessory name when the device is added and keeps its own copy, so a rename shows up there only for accessories you add after it ([#467](https://github.com/RiDDiX/home-assistant-matter-hub/issues/467))
- 🔍 **Editing the entity filter takes effect right away**: a removed entity rode the five minute outage protection meant for Home Assistant restarts. It is removed on the spot now, its endpoint number stays reserved so re-adding restores the device with its groups, and a filtered-out vacuum takes its room switches along and brings them back on their old numbers ([#468](https://github.com/RiDDiX/home-assistant-matter-hub/issues/468))
- 🍳 **An oven exposed as a climate entity mounts in a Fahrenheit install**: registration multiplied the raw Fahrenheit number by 100 before the unit conversion had a chance to run, so a template climate with `max_temp: 500` failed with "Value 50000 is above the int16 maximum". Registration values are clamped into the Matter range now and the thermostat converts them on its first update ([#470](https://github.com/RiDDiX/home-assistant-matter-hub/issues/470))
...
- 🪟 **Covers keep the same Matter fingerprint for life**: a cover that had been moved used to come back from a restart advertising an extra attribute it never had when the controller paired it ([#456](https://github.com/RiDDiX/home-assistant-matter-hub/issues/456))
- 🧩 **New `composedPrimaryOnParent` bridge option, off by default**: puts the primary entity of a self-grouped composed device on the parent endpoint, so Apple Home reads the accessory as a light instead of taking an icon from one of the grouped entities. Needs `autoComposedDevices` on as well. It changes the endpoint layout, so the device has to be removed and added again in the controller, and Apple Home has been seen to stop listing the grouped entities afterwards, so try it on one bridge first ([#469](https://github.com/RiDDiX/home-assistant-matter-hub/issues/469))
- 📡 **New `omitEventsInPriming` bridge option, off by default**: for Google fabrics that acknowledge every chunk of the initial subscription report but never answer the final one, leaving the device permanently offline. With it on, the initial report carries attributes only, live events afterwards are unaffected ([#424](https://github.com/RiDDiX/home-assistant-matter-hub/issues/424))
...
- 🔐 **A controller that refuses the bridge certificates says so in the log**: pairing that stops right after the attestation step left only a bare "Failed" on the controller and nothing in the HAMH log. Controllers that only pair with CSA certified products, like the Ambiscape feature on Philips Ambilight TVs, cannot be used at all ([#465](https://github.com/RiDDiX/home-assistant-matter-hub/issues/465))

**Previously in v2.0.56:**
...
<details>
<summary><strong>🧪 Alpha Features (v2.1.0-alpha.889)</strong> - Click to expand</summary>
