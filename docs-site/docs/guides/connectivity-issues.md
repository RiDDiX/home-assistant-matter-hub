...
- **Port 5540**: Alexa reliably completes pairing only with a bridge on port `5540`. On another port the pairing usually rolls back about 20 seconds after AddNOC, see [#401](https://github.com/RiDDiX/home-assistant-matter-hub/issues/401); a few setups paired on `5541` or `5542` anyway, so treat the port as the first thing to check, not a guarantee. Keep the bridge Alexa pairs with on `5540`.
- **Alexa finds the bridge but never connects**: the Alexa app sits at "Looking for your device" while the bridge log shows no pairing attempt at all. In [#478](https://github.com/RiDDiX/home-assistant-matter-hub/issues/478) one setup had a firewall rule blocking UDP `5540`. Check the firewall for UDP `5540` first, then try the 11-digit manual code shown on the bridge page. If the bridge log never shows `PaseServer Received pairing request` while the app is at "Looking for your device", the Echo did not reach the bridge. Accept the "not Matter certified" prompt when it appears. Some Echo models stop about 80 seconds after that prompt, see [Controllers that only pair with CSA certified products](#controllers-that-only-pair-with-csa-certified-products).
- **Something else on port 80**: two setups only got Alexa past pairing after freeing TCP port 80 on the Home Assistant host, one with the Home Assistant UI moved from `8123` to `80`, one with the Emulated Hue integration holding it ([#449](https://github.com/RiDDiX/home-assistant-matter-hub/issues/449), [#478](https://github.com/RiDDiX/home-assistant-matter-hub/issues/478)). Matter itself never uses port 80 and nobody has explained the connection, so treat this as a thing to try, not a rule. It is worth checking when the Echo shows the "not Matter certified" prompt and then stops, or when pairing never starts at all. Check with `ss -tlnp | grep ':80 '` on the host; the Health Dashboard's network diagnostics also flags it. Move the Home Assistant UI back to `8123`, or stop whatever holds the port, long enough to pair, then put it back. Nothing here says a service on port 80 is wrong, only that freeing it helped twice.
- **Stale device records**: Alexa keys its records on the Matter unique ID, so a device can keep an old name, room or type after you rebuilt the bridge. Delete it in the Alexa app, set **Unique ID Suffix** in the bridge settings (any short text), restart the bridge and run discovery again; every bridged device on that bridge then gets a fresh identity (standard bridge mode only, a standalone device in server mode keeps its id) ([#385](https://github.com/RiDDiX/home-assistant-matter-hub/issues/385)). For many stale entries use the [bulk delete guide](./alexa-bulk-delete-devices.md).
