...
- **Something else on port 80**: two users could only pair Alexa after freeing port 80 on the Home Assistant host, one had moved the HA UI to `80`, the other had Emulated Hue on it ([#449](https://github.com/RiDDiX/home-assistant-matter-hub/issues/449), [#478](https://github.com/RiDDiX/home-assistant-matter-hub/issues/478)). Nobody knows why yet, so it's something to try, not a rule. Check with `ss -tlnp | grep ':80 '`, the Health Dashboard's network check shows it too. Stop whatever holds port 80 while pairing, then turn it back on.
