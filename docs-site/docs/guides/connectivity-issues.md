...
- **Alexa finds the bridge but never connects**: the Alexa app sits at "Looking for your device" while the bridge log shows no pairing attempt at all. The QR code flow then stalls in the Alexa app or Amazon's cloud before the Echo is handed the job. Pair with the 11-digit manual code shown on the bridge page instead of scanning the QR code, accept the "not Matter certified" prompt, and pairing goes through, see [#478](https://github.com/RiDDiX/home-assistant-matter-hub/issues/478). Some Echo models still stop about 80 seconds after that prompt, see [Controllers that only pair with CSA certified products](#controllers-that-only-pair-with-csa-certified-products).
