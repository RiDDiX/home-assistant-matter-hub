...
- **Alexa finds the bridge but never connects**: the Alexa app sits at "Looking for your device" while the bridge log shows no pairing attempt at all. In [#478](https://github.com/RiDDiX/home-assistant-matter-hub/issues/478) one setup had a firewall rule blocking UDP `5540`, the other started working with a freshly created bridge. Check the firewall for UDP `5540` first, then try the 11-digit manual code shown on the bridge page, then a new bridge on `5540`. Accept the "not Matter certified" prompt when it appears. Some Echo models stop about 80 seconds after that prompt, see [Controllers that only pair with CSA certified products](#controllers-that-only-pair-with-csa-certified-products).
