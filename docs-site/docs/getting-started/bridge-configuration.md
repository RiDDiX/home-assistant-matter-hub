...
| `composedPrimaryOnParent` | Applies to devices you group yourself with **Composed Sub-Entities** in the entity mapping, and needs `autoComposedDevices` on as well. Puts the primary entity on the parent endpoint instead of on an extra sub-endpoint, so the accessory announces its own device type. Without it the parent is a bare bridged node and Apple Home takes the accessory icon from one of the grouped entities, for example an outlet for a composed light. Automatically composed sensors, air purifiers and climate/fan devices are not affected. Changes the endpoint layout, so restart and re-add the grouped devices in your controller, and try it on one bridge first: Apple Home has been seen to stop listing the grouped entities once the parent carries its own device type ([#469](https://github.com/RiDDiX/home-assistant-matter-hub/issues/469)) | `false` |
| `omitEventsInPriming` | Google offline workaround: send the initial subscription report without the stored StartUp and BootReason events. Google has been seen to acknowledge every chunk of that report on the wire but never answer the final one, which carries exactly these events, so the subscription aborts after about 11 seconds and the device stays offline. Live events afterwards are unaffected. Technically off-spec, only enable it on a bridge paired to an affected Google fabric ([#424](https://github.com/RiDDiX/home-assistant-matter-hub/issues/424)) | `false` |
